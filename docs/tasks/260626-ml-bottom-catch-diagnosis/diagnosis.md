...
本报告 §3.3–§8 早期使用了**"事件级几何比对"口径**评估抓底 Precision/Recall：即"模型信号日期 vs swing low 中心 ± k 天"做几何距离判断。

**这个口径与训练标签（label 列）不一致**——模型训练时的真值是 `sl_proximity` 生成的 `label` 列（信号日落在 swing-low±k 内则 label=1）。评估应当**直接用 label 列**（训练口径=评估口径），而不是另造一套几何比对。

**口径错误导致的错误结论**：几何比对夸大了低 N 的 Recall，一度推出"N=1 最优"。**用 label 列重算后结论翻转：N=2 才最优**（候选池正样本率最高 41.2%、@0.35 时 Precision/Recall 双优）。

**正解见 §11（label 口径重评）。** §3.3–§8 的几何比对数字保留作为"弯路记录"，但**结论以 §11 为准**。

**代码层面（2026-06-27 二次更正）**：经逐脚本核查，真正用几何比对的只有 `grid_search_n_threshold.py` 与 `plot_n1_vs_n2.py` 两个（早期错误结论来源于此）；`sweep_ndays_precision_recall.py`、`plot_precision_recall.py` 其实一直用 label 列、口径正确。**现已把那两个几何脚本真正改写为 label 口径**（不是加注释了事），所有研究脚本口径统一，结论一致指向 **N=2 @0.35**。`sweep_threshold_event.py` 的 P/R 也改为 label 口径，并保留口径无关的"信号簇"洞察。

不受影响、依然成立的结论：
- 评估不能用收益率（SPY 长牛偏置）—— 对
- 阈值 0.35（阈值不依赖 GT 口径）—— 对
- XGBoost 无需特征分箱 —— 对
- §10 EMA 连续天数 bug 修复 —— 对
- **N=2 为生产门槛**（label 口径验证）—— 对

---

## 0. 一句话结论

1. **"ROI 没增益"不是 ML 的锅，是评估尺子用错了**——SPY 长牛，买越多越赚，任何"减少买入"的过滤器都会系统性拉低收益率。用抓底命中率（Precision）评估，ML 是**有效**的。
2. **生产门槛 = N=2**（label 口径验证，§11）。N=2 候选池正样本率最高（41.2%），@阈值0.35 时 Precision(46.5%)/Recall(80%) 双优。⚠️ 早期几何口径曾误推 N=1，已更正（见顶部声明）。
3. **生产阈值 = 0.35**（钉死，不灵活变动，§9.1）。跨 N 都在峰值区，最稳健。
4. **XGBoost 无需特征分箱**；阈值不该用 0.5（详见 §5）。
5. **关键洞察（不受口径影响）**：阈值过低反而造成最严重的连续加仓（底部区域连续多天报底黏成大簇），适度抬高阈值让信号回归稀疏。
6. **生产配置 = `v3_n2`（N=2, 阈值0.35）**，已是 ACTIVE_VERSION，无需改动。
...
> ⚠️ 本节及 §3.4/§3.5 的具体 P/R 数字用的是**几何比对口径**（信号日 vs swing low 中心 ±k），与 label 列不一致，偏乐观。**"ML 有效"这个定性结论成立**，但门槛对比数字以 §11 label 口径为准。
...
> ⚠️ **本节基于几何比对口径，结论已被 §11（label 口径）更正。最终结论：N=2。** 以下保留作弯路记录。

| 取向 | 推荐 N | 理由 |
|------|:---:|------|
| 宁可多抓、少错过（DCA 风格） | ~~N=2 或 N=3~~ | ~~抓底质量与 N=5 基本一致，但保留远多的底部机会~~（几何口径，已更正） |

> ~~结论：各 N 抓底质量曲线几乎重合~~ —— **此判断错误**。label 口径下各 N 抓底质量并不相同，N=2 候选最纯、P/R 最优（详见 §11）。
...
> ⚠️ 本节的事件级 P/R 数字用**几何比对口径**（与 label 不一致），数字偏乐观。但**阈值结论（0.35、"阈值过低反致连续加仓"）不依赖 GT 口径，依然成立**。门槛/P/R 对比以 §11 为准。
...
### 9.3 实测（6/25 收盘，剔除 6/26 盘中数据）

> 注：下表 16.6% 是**修 EMA bug 之前**的旧值。修复后（当日收盘口径）连续天数 6/25=3，概率 = **15.1%**（见 §10.5、§11.3）。
...
| 是否信号日 | ✅ 是信号日（满2天） |
| 底部概率 | **15.1%** |
| 卡 0.35 | 15.1% < 0.35 → **不加仓** |

**结论**：6/25 概率 15.1% 远低于 0.35 阈值 → 当前不是底，不加仓。SHAP 显示主因是 RSI 未超卖 + 市场宽度健康。此结论与 MMA 结构判断一致（跌得不够深/不够恐慌）。

---

## 10. Bug 修复：连续低于 EMA20 天数滞后一天（口径修正）

### 10.1 现象
TradingView 上 6/23、6/24、6/25 三天收盘都在 EMA20 下方（应为连续 3 天），但系统显示"连续 2 天"。怀疑数据/EMA 没对齐。

### 10.2 系统化排查（systematic-debugging）
| 假设 | 证据 | 结论 |
|------|------|------|
| 数据没对齐 | 系统 6/25 EMA20=739.62 ≈ TV 的 739.5 | ❌ 证伪 |
| EMA 算错 | 值正确 | ❌ 证伪 |
| 计数逻辑滞后 | 逐日 trace 计数器 | ✅ **确认根因** |

**根因**：`compute_backtest_ema_context`（和 `BacktestEngine`）用**前一日收盘**与当日 EMA 比较来累加 `consecutive_below_ema`。
- 6/23 收盘 733.58 已跌破 EMA，但判定时用的是 6/22 收盘 744.39（站上 EMA）→ 6/23 计数仍为 0，真实跌破推迟到 6/24 才计入。
- 结果：系统连续天数 = 真实跌破天数 − 1（滞后一天）。

注：6/22 收盘 744.39 > EMA20 741.68，确实站上 EMA，打断了更早的连续性 —— 这部分系统是对的。

### 10.3 是不是 bug？取决于交易时点
- 旧"前一日收盘"口径适合**盘中/开盘下单**（当日收盘未知，只能用昨收）。
- 本策略实盘是**当日收盘决策**（尾盘/按收盘价下单），当日收盘已知，应使用**当日收盘**口径 → 旧实现对本策略是错的。

### 10.4 修复（口径统一为"当日收盘"）
1. `ml/labeling.py::compute_backtest_ema_context`：`prev_close` → 当日收盘。
2. `src/backtest_engine.py`：连续天数判定同步改为当日收盘（保留 `ctx.prev_close` 供其他策略），避免训练/回测口径裂缝。
3. 重训 v3_n2（信号集随口径变化：样本 2224→2286）。
4. `predict.py` 写死的 "NDay5" 文案动态化为 NDay{n_days}。

### 10.5 验证
修复后 6/23=1、6/24=2、**6/25=3**，与 TradingView 看图一致。6/25 实测：是信号日，底部概率 15.1% < 0.35 → 不加仓（结论不变）。

**教训**：`compute_backtest_ema_context` 与 `BacktestEngine` 必须口径一致（前者本就是为复刻后者而存在），任何一方改动都要同步另一方，否则信号日期漂移。

---

## 11. 口径纠错与最终定论（label 列，权威结论）

### 11.1 错误根源：评估口径 ≠ 训练口径

之前用"几何比对"（信号日 vs swing low 中心 ±k 天）算 P/R，**自造了一套真值**。而模型训练的真值是 `sl_proximity` 生成的 **label 列**。两者有出入：几何比对把"信号靠近 swing-low 中心"也算抓到，**夸大了低 N 的 Recall**，误推出"N=1 最优"。

**正确做法**：直接用 label 列（训练口径=评估口径），逐样本比对模型预测 vs label。

### 11.2 label 口径重评（N=1..5，walk-forward 样本外，修 bug 后当日收盘口径）

| N | 候选 | 真底(label=1) | 基线P | AUC | @0.30 P/R | @0.35 P/R | @0.40 P/R |
|---:|---:|---:|---:|---:|---|---|---|
| 1 | 1691 | 669 | 39.6% | 0.592 | 44.2%/86% | 44.9%/78% | 44.9%/70% |
| **2** | 1372 | 566 | **41.2%** | 0.587 | 45.3%/87% | **46.5%/80%** | 47.2%/73% |
| 3 | 1151 | 477 | 41.4% | 0.563 | 45.2%/87% | 45.2%/79% | 46.3%/73% |
| 4 | 982 | 406 | 41.3% | 0.587 | 45.2%/82% | 45.3%/75% | 46.7%/69% |
| 5 | 847 | 351 | 39.9% | 0.588 | 46.7%/81% | 46.8%/72% | 47.9%/64% |

复现：见附录脚本（用 label 列算 P/R，非几何比对）。

### 11.3 最终定论

1. **门槛 N=2**：
   - 候选池正样本率（基线P）**最高 41.2%** —— 证实"真底多在连跌≥2天后"，N=2 候选本身就更靠近底部。
   - @阈值0.35：Precision 46.5% / Recall 80%，**在准与全两项上 ≥ N=1**（N=1 是 44.9%/78%）。
   - 用对口径后，几何比对鼓吹的"N=1 高 recall"消失了（实际 N=1 recall 78% < N=2 的 80%）。
2. **阈值 0.35**：跨所有 N 都在峰值区，稳健。
3. **生产配置 `v3_n2`（N=2, 0.35）无需改动**——当前模型正是 N=2 训练，与最优一致。

### 11.4 教训（写给未来的自己）

> **评估指标的"真值"必须和训练标签同源。** 任何"另造一套 GT 来评估"的做法（哪怕看起来等价，如几何 ±k 比对）都可能与 label 列有微妙出入，导致**相反的错误结论**。验证模型，永远优先直接用训练时那个 label。
